; DV3 MSDOS FAT16 Check Format	      V3.02	      2017 W. Lenerz
; 2020-03-08  3.02  modified for correct name in ddf_mname (MK+wl)
; based on
...
;	d7	total sectors
	...
	move.b	(a2)+,d2
	lsr.w	#1,d2			; n*512?
	bcs.s	mck_badn		; d2 =nbr of 512 byes in 1 sector, should be 1
	moveq	#0,d6
	move.b	(a2),d6 		; cluster size
	ble.s	mck_badl		; ... zero
	cmp.b	#8,d6
;****	     bgt.s   mck_badn		     ; ... too large
	bne.s	mck_badn		; must be this large
	moveq	#$f,d0
	...
	move.w	d7,d0			; total sectors (small numbers)
mck_badl
	blt	mck_bad 		; no negative values!
	bgt.s	ok1			; sectors small numbers ok
	move.l	$20(a1),d0		; get big sectors number
	rol.w	#8,d0
	swap	d0
	rol.w	#8,d0
	move.l	d0,d7
ok1	moveq	#0,d3			; preset no track format
	moveq	#0,d4
	moveq	#0,d5
...
*******        mulu    d2,d7		       ;	sectors / drive
	...
	move.l	d7,d1			; total sectors on disk
     ;	 divu	 d6,d1			 ; total allocation units
     ;	 move.w  d1,d0
	lsr.l	#3,d1			; cluster size is 8 ****************
	move.l	d1,(a2)+		; number of clusters
...
mck_badl2
	...
	mulu	d2,d4			; real number of FAT sectors
					; (when counting as 512 bytes sectors)
	moveq	#0,d5
	...
	move.l	d6,d0			; save sectors per cluster
	move.l	d3,d6			; reserved sectors
	add.w	d5,d6			; + map
	add.w	d2,d6			; + root directory
	sub.w	ddf_asect(a4),d6	; - 1 cluster
	sub.w	ddf_asect(a4),d6	; DOS groups start at 2!!

					; d7 = total sectos on disk
	sub.l	d6,d7			; the number of sectors alloc by FAT
	lsr.l	#3,d7			; ... nbr of groups in the FAT ********

mck_4nib
	lea	msd_table,a2		; reset vector table
	jsr	dv3_setfd
...
mck_redef
	...
	move.l	d3,-(a7)
	moveq	#9,d3			; value to shift for 512 bytes sectors
...
;	 divu	 ddf_slen(a4),d0
	...
	addq.l	#2,d0			; room for complete sectors + root

	move.l	d0,d1
	add.l	d2,d0			 ; + directory
	move.l	d0,d2
