; Atari Hard Disk Control Procedures   V1.02    1990	Tony Tebby QJUMP
...
wd$.reg reg	d1/d2/d3/a1/a2/a3
	...
	move.l	ddl_slave-ddl_thing(a2),d2
	...
	beq.s	wd$_done		 ; ... no
	move.l	d2,a3
	lea	hdl_targ+7(a3),a3	 ; target of slave
	add.l	d3,a3
	moveq	#8,d0			 ; target+8
	move.b	hdl_part-hdl_targ(a3),d1 ; partition
	bmi.s	wd$_done		 ; ... none
...
hdt_setd
	lea	-ddl_thing(a2),a3	 ; get master linkage

	moveq	#$f,d1			 ; 8+ are SCSI
	and.l	(a1)+,d1		 ; target
	move.l	ddl_slave(a3),a4	 ; any slave?
	add.w	d7,a3			 ; index tables
	st	hdl_part-1(a3)		 ; get rid of ASCI definition
	move.l	a4,d0
	beq.s	hdt_acsi		 ; ... no
	add.w	d7,a4
	st	hdl_part-1(a4)		 ; get rid of SCSI definition
	bclr	#3,d1			 ; clear SCSI flag
	beq.s	hdt_setp		 ; not SCSI, set it
	move.l	a4,a3			 ; set SCSI instead

hdt_acsi
	bclr	#3,d1			 ; clear SCSI flag
	bne.s	hdt_rtok		 ; SCSI, forget it
hdt_setp
...
win_remv thg_extn {REMV},win_wp,hd_bochr
	...
	bsr.s	hdt_prep
	lea	hdl_remd(a3),a0
	tst.b	hdl_part-hdl_remd-1(a0,d7.l) ; defined?
	bmi.s	hdt_fdnf		 ; ... no
	moveq	#0,d3
	move.b	hdl_targ-hdl_remd-1(a0,d7.l),d3 ; target
...
hdt_setr
	move.b	d5,hdl_remd-hdl_remd(a0,d3.l) ; set remove flag
...
win_slug thg_extn {SLUG},,hd_byte
	move.w	2(a1),hdl_paus-ddl_thing(a2) ; set integer
	moveq	#0,d0
	rts
	end
