; DV3 QL Format Fetch Directory Entry	 V3.01		 2017 W. Lenerz
; based on
; DV3 QL Format Fetch Directory Entry	 V3.00		 1992 Tony Tebby
	...
	xref	qw1_dlen			; calculate directory length
...
;+++
; DV3 Qubide Format Scrumple Directory Entry at A2
;
...
;+++
; DV3 Qubide Format Fetch Directory Entry For File
;
...
;+++
; DV3 Qubide Format Fetch Directory Entry
;
;	d0 cr	format type required / status return
...
qde_fent
	cmp.b	#ddf.qdos,d0		 ; check for standard or Qubide
	sne	-(sp)
	moveq	#1,d5			 ; entry 0 is at position 1
	add.l	d2,d5
	moveq	#7-hdr.sft,d0		 ; header to sector
	add.b	ddf_slflag(a4),d0	 ; + sector length flag
	lsr.l	d0,d5			 ; directory sector containing header of file

	jsr	(a2)			 ; locate appropriate directory sector
	bne.l	qde_bad

	addq.l	#1,d2
	lsl.w	#hdr.sft,d2
	and.w	ddf_smask+2(a4),d2	 ; byte within sector
	add.w	d2,a2			 ; start of data (header of file)

	tst.b	(sp)+			 ; scrumple or copy?
	beq.s	qde_copy

** i should never get to this part of hte code (?!!!!)
qde_scrumple
	move.l	(a2)+,d3c_flen(a0)	 ; length
	move.w	(a2)+,d0		 ; type in lsb
	cmp.b	#hdrt.dir,d0		 ; directory?
	bne.s	qde_sattr		 ; no
	or.w	#dos.subd<<8,d0 	 ; set dos sub-directory attribute (why?)
qde_sattr
	...
	lea	d3c_fnam(a0),a1
	move.w	(a2)+,d0		 ; length of name in directory
	beq.s	qde_none
...
qde_copy
	tst.w	hdr_flid(a2)
	beq.s	qde_none
	cmp.b	#-1,5(a2)		 ; is this a subdir?
	bne.s	qde_ndir		 ; no
	move.w	hdr_flid(a2),d1 	 ; file ID
	jsr	qw1_dlen		 ; get file length into d0
	move.l	d0,(a1)+
	addq.l	#4,a2
	bra.s	qde_cp2

qde_ndir
	move.l	(a2)+,(a1)+		 ; file length
qde_cp2
;	 clr.l	 -4(a1)
