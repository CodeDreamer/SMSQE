; DV3 QUBIDE Make Directory	    V3.02	    2017 W. Lenerz
; based on
; DV3 QL Make Directory 	    V3.00	    1992 Tony Tebby
; 2014.06.01  3.01  check for open files before making dir (wl)
; 2017.06.09  3.02  adapted for Qubide (wl)


; in Qubide, the fat entries are : file_nbr (word) file_block (word)
; so, contrary to qlwa format, the file number in the directory doesn't point
; to an entry in the fat, but contains the file number. Hence, the file number
; doesn't change when moving files about.
...
qdm.reg reg	d1/d2/d3/d4/d5/a1/a2/a5
	movem.l qdm.reg,-(sp)
	jsr	qw1_drloc		; get my directory entry (into A2)
...
qdm.mreg reg	d4/d5/a1/a2
	...
	bne.s	qdm_fbloc		; there's still space, locate (directory) file buffer
...
; (under qubide, like under msdos, the file id doesn't change)
...
qdm_nxfile
	...
	moveq	#0,d4
	addq.w	#1,d5
	jsr	dv3_drloc		 ; yes, get next sector
	bra.l	qdm_floop
...
;------------------------------- 3.01
; this checks whether any file which should be moved into the newly created
