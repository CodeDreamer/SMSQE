* Network remote filing OPEN/CLOSE  V0.5    1985   Tony Tebby	 QJUMP
*
* 2018-08-06  0.5  Changed add.w #io.open to add.l, like it is in GC ROM (MK)
...
nfo_use
	...
	lea	ndd_nunr-ndd.nuln(a3),a5
	add.w	d6,a5
	move.w	(a5)+,d6		get net node number
	move.w	(a5)+,d0		and length of use name
*
nfo_usel
	move.b	(a5)+,(a1)+		copy use name
	subq.w	#1,d0
	bgt.s	nfo_usel
*
nfo_name
	lea	fs_fname(a4),a5 	now copy characters of name
	move.w	(a5)+,d0
	bra.s	nfo_nend
nfo_naml
	move.b	(a5)+,(a1)+
...
;;;	   add.w   #io.open<<8,d1	   set type to OPEN
	add.l	#io.open<<8,d1		set type to OPEN
	move.l	d1,(a2) 		set block number, type and length
*
	bsr.s	nf_sendw		send packet and get reply (waiting)
	bne.s	nfo_rel 		... oops
...
*
nu_close
	sub.w	#ndd.lenf,a3		real linkage block
nf_close
	movem.l d7/a4/a5,-(sp)		save regs
	move.l	a0,a4
