; Allocate in common heap  V2.02    1994  Tony Tebby
; Fast RAM in High Memory

	section mem

	xdef	mem_achp
	xdef	mem_atpa		 ; is the same
...
;	d3 c  p 'ACSI' for ACSI DMA memory
...
stk_d1	equ	$00
	...
	ble.l	mac_imem
	cmp.l	#'ACSI',d3		 ; ACSI memory requested?
	beq.s	mac_chp 		 ; ... yes, allocate in common heap
	lea	sys_tpaf(a6),a0 	 ; check for room in TPA
	tst.l	(a0)			 ; any?
	beq.s	mac_chp 		 ; ... no
	bsr.l	mem_albu		 ; ... allocate from bottom up
	beq.s	mac_done
mac_chp
