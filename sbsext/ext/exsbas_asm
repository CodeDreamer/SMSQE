* EX, EW and ET V2.00	 1984 Sinclair Research and Tony Tebby  QJUMP
...
ex_ew_et
	clr.l	-(sp)		save the SBAS flag / channel ID
	...
	clr.l	-(sp)			; future pointer to filename
*
...
*
prg_loop
	clr.l	job_sbas(sp)	clear sbas pointer
...
prog_found
	moveq	#ioa.kshr,d3
	moveq	#uod.extn+uod.prgd,d2 ; program default
	lea	uxt_bopt,a2	      ; optional basic extensions
...
prog_sbst
	...
	move.l	(a5),a5
	move.l	sb_vars+sb_sbjob(a5),d0 start address
	rts

prog_sbas
	move.l	a0,job_sbas(sp) it's SBASIC
	bsr.s	prog_sbst	set start address
	move.l	d0,a1
	move.l	#sb.jobsz,d0	data space
	moveq	#$10,d2 	program size
...
* !!!
* additions for "home" thing
	move.l	file_nm(sp),a1		; filename to set
	move	sr,d4
	trap	#0			; supervisor mode for absolute a1
	add.l	a6,a1			; now absolute
...
ex_sbload
	moveq	#ioa.sown,d0
	trap	#2		set channel owned by job

	bsr.s	prog_sbst	set start address
	move.l	d0,a0
	move.l	(a0)+,(a1)+	bra.s; 0000
	move.l	(a0)+,(a1)+	00004afb
	move.l	(a0)+,(a1)+	0006'SB'
	move.l	(a0)+,(a1)+	'ASIC'
	clr.l	(a1)+		... no second flag
...
ex_fend
	move.l	job_sbas(sp),d0 is there a BASIC source file?
	beq.s	ex_snchn	... no
	move.l	d0,-(a4)
	addq.w	#1,d5
ex_snchn
...
*
;;;	   moveq   #25,d3	   suspend for .5 of a second, to give job a chance
;;;	   bsr.s   ex_proc
;;;	   sub.b   #$30,d0	   030?
;;;	   blt.s   ex_susp
;;;	   beq.s   ex_susp30
...
;;;	   moveq   #5,d3
;;;ex_susp
	jsr	gu_exdelay	; get system specific execution delay
	move.l	d0,d3
...
;;;	   moveq   #sms.xtop,d0
;;;	   trap    #1
;;;	   moveq   #70,d0	; Is the 7 here by mistake? Doesn't make sense
;;;	   add.b   sys_ptyp(a6),d0
;;;	   rts
