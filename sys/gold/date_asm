...
gl_rdate
	...
	bsr.l	gl_rtc_status		; RTC ok?
	bne.s	grd_ql			; ... no, read QL date
	tst.b	sgx_work+sgo_prtc	; protected date?
	beq.s	grd_do			; ... no, read RTC date
grd_ql
...
grd_loop
	move.l	glk.base+glx_ptch,a1
	move.w	glo_rtcs(a1),d7
	swap	d7			 ; am / pm
...
;---
gl_sdate
gsd.reg  reg	d1/d2/d3/d4/d6/d7/a0/a1/a2
stk_d1	equ	$00
	movem.l gsd.reg,-(sp)
...
gsd_sqel
	...
	moveq	#0,d0
	tst.b	sgo_prtc+sgx_work	 ; protect the clock?
	bne.l	gsd_exit
	tst.w	glk.card+glx_ptch	 ; type of card
	beq.l	gsd_sgc 		 ; ... SGC
	moveq	#10,d6			 ; try several times
gsd_rtry
	move.l	glk.base+glx_ptch,a2	 ; hardware base $1c000

	move.b	#glk.rtcs,glo_rtcc(a2)	 ; stop the clock
	lea	glo_s01(a2),a1
	move.l	stk_d1(sp),d1		 ; restore time
...
gsd_putl
	...
	bsr.l	gl_rtc_fail
gsd_putx
	addq.l	#glk.rtca,a1		 ; next time/date register
...
; ***** SGC version of set date code
gsd_sgc
	moveq	#10,d6
gss_rtry
	move.l	glk.base+glx_ptch,a2
	or.w	#$0700,sr
	sf	glo_rtcc(a2)
	sf	glo_dow(a2)
	sf	glo_d01(a2)
	...
	rol.l	#1,d1			 ; seconds in upper d1 (ovf in lsb)
	moveq	#6,d2
	bsr.l	gss_divp		 ; divide upper d1 by 10 and d2 and put
	bsr.l	gss_divp		 ; twice
	clr.w	d1
	swap	d1			 ; hours are the remainder
	divu	#12,d1
	move.w	d1,d7			 ; 0/1 in d7 is am/pm
...
gss_divp
	swap	d1
	divu	#10,d1
	bsr.s	gss_putm		; next nibble is 0-9
	swap	d1
	divu	d2,d1			; get 10s
...
gss_put
	...
	and.b	#$f0,d1 		;!!!!!!
gss_putl
	or.w	#$0700,sr		; RTC write protection on SGC
	sf	glo_rtcc(a2)		; write $1c000
	sf	glo_dow(a2)		; write $1c038
	sf	glo_d01(a2)		; write $1c020
	move.b	d1,(a1) 		; write data byte
	not.b	d1
	sf	glo_rtcc(a2)
	sf	glo_dow(a2)
	sf	glo_d01(a2)
	move.b	d1,glo_ten(a2)		; $1c004. This is read-only, so ???
	and.w	#$f8ff,sr
	not.b	d1
	swap	d4
	move.b	#$f0,d4
	and.w	(a1),d4 		; write successful?
	cmp.b	d4,d1
	beq.s	gss_putx		; ... yes
	swap	d4
	dbra	d4,gss_putl		; ... no, try again

	lea	sgx_work+sgo_rtcs,a0	; RTC setting failed!
	bsr.l	gl_rtc_fail

gss_putx
	addq.l	#glk.rtca,a1		; next register
	clr.w	d1			; we've used this bit
	rts
