; Buffering (get byte parity set) utility  V2.00    1989  Tony Tebby   QJUMP
...
;+++
; This routine gets a single byte from an IO buffer and sets the parity
; according to the parity code in d7. If the byte is in a new buffer,
; the old buffer is added to the linked list of throwaways.
