; Buffering (get byte with parity) utility  V2.00    1989  Tony Tebby   QJUMP
...
;+++
; This routine gets a single byte from an IO buffer and checks that the
; parity is correct and returns the appropriate error. If the byte is in a
...
;
;       d0  r   status: 0, err.nc, err.prty or +1 for end of file
...
ibgb_dyn
        cmp.l   buf_nxtp(a2),a3          ; at end
        blt.s   ibgb_gbf                 ; get byte
        cmp.l   buf_endb(a2),a3          ; off end?
        blt.s   ibgb_nc                  ; ... no
...
ibgb_ok
        ...
        move.b  d7,d0
        move.l  (sp)+,a3
        bne.l   iob_prtc                 ; exit to parity check
        rts
