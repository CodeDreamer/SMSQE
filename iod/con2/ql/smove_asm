; Screen move				v2.01   1991 Tony Tebby
;						 2003 Marcel Kilgus
;
; 2003-01-15  2.01  Serious bug fixed that showed up when the screen was the
;		    destination (mostly the case in low memory situations) (MK)
;
...
psm_slck
	...
	lsr.w	#2,d0			; to number of bytes	&&&& was asr
	and.w	#$fffc,d0		; to nearest long word
	move.w	d0,a2			; that's row increment
;
	move.l	#pt.samsk,d0
	and.l	sd_xhito(a1),d0 	; x-origin is offset this much in save
...
;
	...
	move.w	pt_bytel-iod_iolk(a3),a3 ; and row increment
