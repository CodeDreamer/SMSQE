...
last_word
	ror.l	d7,d4
	move.l	0(a1),d0		and set the composite mask
	lsr.l	#8,d0
	lsl.w	#8,d0
	lsr.l	#8,d0
	and.w	d0,d4
	...
	move.l	(a0),d1 		red pattern
	lsl.w	#8,d1
	lsr.l	#8,d1
	and.w	d4,d1			mask pattern (red first)
	move.l	(a3),d0 		get current red background from screen
	lsl.w	#8,d0
	lsr.l	#8,d0
	not.w	d4
	and.w	d4,d0
	not.w	d4
	eor.w	d1,d0
	move.l	(a0),d1 	       now green
	lsr.l	#8,d1
	lsl.w	#8,d1
	lsr.l	#8,d1
	and.w	d4,d1
	not.w	d4
	move.b	d0,3(a3)		replace red
	lsr.w	#8,d0
	move.b	d0,1(a3)
