...
;
	...
	add.w	d3,a1
	add.w	d3,a1			; word pixels
;
	tst.l	d2			; are we scanning?
	bpl	ptr_crpx		; no, just read pixel
;
	cmp.b	#32,sd_wmode(a0)	; mode 32?
	bne.s	ptr_nm32		; no
	ror.w	#8,d2			; make colour little endian
ptr_nm32
	btst	#iop..gcl,d2		; were we given a colour?
	bne.s	ptr_fgcl		; yes, just find it
	move.w	(a1),d2 		; else set current as "parameter"
ptr_fgcl
...
;
	moveq	#-2,d4			; west pixel displacement
...
;
ptr_crpx
	move.w	(a1),d1 		; return pixel colour
ptr_chkm
	cmp.b	#32,sd_wmode(a0)	; mode 32?
	bne.s	ptr_exok		; no
	ror.w	#8,d1			; return colour big endian
ptr_exok
