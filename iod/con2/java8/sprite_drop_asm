...
sp_drop
	...
	move.l	(a5),a5
	move.l	a5,d4
	beq.s	sp_dropi	; ... sprite mask ptr is 0
...
sp_line
	...
	bne.s	odd		; yes, address byte by byte
;
long_word
	move.l	(a5)+,d0
	not.l	d0
...
odd	move.l	(a5)+,d0	; 1st byte
	not.l	d0
	move.b	(a3),d1
	swap	d1		; = left shift by 16 bits
	move.w	1(a3),d1	; 2nd + 3rd bytes
	lsl.l	#8,d1
	move.b	3(a3),d1	; 4th byte
	and.l	d1,d0
	move.l	(a4)+,d1
	eor.l	d1,d0
	move.b	d0,3(a3)
	lsr.l	#8,d0
	move.w	d0,1(a3)
	swap	d0		;  = right shift by 16 bits
	move.b	d0,(a3)
	addq.l	#4,a3
	subq.w	#1,d4		; next long word
